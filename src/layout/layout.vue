    ...
    <div class="d2-layout-header-aside-content"
      ...
      <div class="d2-theme-header"
        ...
        <div class="logo-group"
             ...
             flex-box="0">
          <div v-if="asideCollapse">{{ appInfo.shortName }}</div>
          <div v-else>
            <strong>{{ appInfo.name }}</strong>({{ appInfo.shortName }})
          </div>
...
export default {
  ...
  computed: {
    ...mapState('emdata/system', ['appInfo']),
    ...mapState('d2admin', {
      ...
      menus.forEach((menu) => {
        ...
        if (menu.items) {
          menu.items.forEach((item) => {
            ...
            let sitem = Object.assign({}, item)
            sitem.title = `${menu.title}-${item.title}`
            sitems.push(sitem)
          })
        }
      })
      this.$store.commit('d2admin/menu/asideSet', menus)
      this.$store.commit('d2admin/search/init', sitems)
    }
  }
}
</script>
